...
def project_finances(income, expenses, loan_amount, annual_rate, loan_years):
    ...
    if monthly_savings < 0:
        ...
        print(f"   Consider a lower loan amount or longer tenure.")
        return

    # Calculate month when loan is paid off
    debt_free_month = months
    ...
    fig.suptitle('LoanSense — 5-Year Financial Projection', 
                 fontsize=16, fontweight='bold')

    # Chart 1 — EMI vs Savings per month
    ax1.bar(month_list, emi_burden_list, label='EMI Burden', 
            color='#e74c3c', alpha=0.7)
    ax1.bar(month_list, savings_list, bottom=emi_burden_list,
            label='Monthly Savings', color='#2ecc71', alpha=0.7)
    ax1.set_xlabel('Month')
    ax1.set_ylabel('Amount (₹)')
    ax1.set_title('Monthly EMI Burden vs Savings')
    ax1.legend()

    # Chart 2 — Cumulative savings over 5 years
    ax2.plot(month_list, cumulative_savings, color='#3498db', 
